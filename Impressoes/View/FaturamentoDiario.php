...
?>      
                ...
                <table class="table table-borderless" style="margin-top:-20px; border: none;   width: 100%;">  
                    ...
                    <td colspan="" style="width: 30%; text-align: right; font-size: 14pt; ">
                        <h4>Faturamento Diário - <?= $_GET['dia']; ?>/<?= mostraMes($_GET['mes']) ?>/<?= $_GET['ano']; ?> </h4>
...
<table class="table table-bordered" style="width: 90%; margin-left: 5%; margin-right: 5%;">  
    ...
    $listaNotasDia = $notasController->RetornarNotasDiaMesAnoF(2, 1, $dia_hoje, $mes_hoje, $ano_hoje);
    ...
    <tr style='text-align: center;'>    
        ...
        <td><b>Valor</b></td>
        <td colspan='3'><b>Tipo Pagamento</b></td>
    </tr>
    <?php
            ...
            if ($listaPagamentos != null) {
                foreach ($listaPagamentos as $user5) {
                    ...
                    if ($user5->getTipo() == 1) {
                        ...
                        if ($user5->getTipopag() == 1) {
                            $totalfinaldiatipopag1 = $totalfinaldiatipopag1 + $total;
                            $textotipopag = "Pagamento á vista no Dinheiro";
                        } else if ($user5->getTipopag() == 2) {
                            $totalfinaldiatipopag2 = $totalfinaldiatipopag2 + $total;
                            $textotipopag = "Pagamento á vista no Cartão de Débito";
                        }else if ($user5->getTipopag() == 3) {
                            $totalfinaldiatipopag5 = $totalfinaldiatipopag5 + $total;
                            $textotipopag = "Pagamento á vista no Pix";
                        }
                    } else if ($user5->getTipo() == 2) {
                        $totalfinaldiatipo2 = $totalfinaldiatipo2 + $total;
                        ...
                        if ($user5->getTipopag() == 1) {
                            $totalfinaldiatipopag3 = $totalfinaldiatipopag3 + $total;
                            $textotipopag = "Pagamento parcelado no Cartão de Crédito";
                        } else if ($user5->getTipopag() == 2) {
                            $totalfinaldiatipopag4 = $totalfinaldiatipopag4 + $total;
                            $textotipopag = "Pagamento parcelado no Crediário";
                        }
                    }

                    $usuarionota = $user4->getUsuario();
                    if($usuarionota==0){
                    $nomeCli = $user4->getNomeCli();
                    }else{
                    $nomeCli = $clienteController->RetornarNomeClientes($usuarionota);    
                    }
                    
                    ?>
                    <tr style='text-align: center;'>
                        <td><?= $user5->getCod(); ?></td>
                        <td colspan='3'><?= retiraAcentos($nomeCli); ?></td>
                        <td>R$ <?= number_format($total, 2, ',', '.') ?></td>
                        <td colspan='3'><?= $textotipopag;?></td>
                    </tr>
                <?php
            }
        }
        ...
        ?>
            ...
            <?= $usu->getNome(); ?> 
                ...
                <th>Total:</th>
                    ...
                    $totalusu = 0;
                    $filtro = $usu->getCod();
                    $listaNotas = $notasController->RetornarNotasDiaMesAno(1, $filtro, $dia_hoje, $mes_hoje, $ano_hoje);
