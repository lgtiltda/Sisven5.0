    ...
    if ($dataTableTestePag != null) {
      ...
      foreach ($dataTablePedidos22 as $resultadopedidos22) {
        ...
        if ($status == 1) {
          ...
          echo "           
                  <a class='btn btn-outline-primary' onclick='ConfirmarParcelamento(75, $codnota, $codpedido, 75$codpedido) ' href='javascript: func' type='button' data-bs-toggle='modal' data-bs-target='#exampleModalDetalhesPedido' >
                        Pagar Parcela   
                        </a>
                        ";
        }
    ...
    echo "
          <div class='row' style='background-color:#fff; margin-bottom:10px;'>
            <div class='col-sm-12 col-xl-3'>
              ...
              <select onchange='validacaorelatorios(46, this.value, txtTipopagamentoAvista2.value, txtTipopagamento2.value, txtDataParaPesquisa222.value, txtTipoRelatorio222, 0, 46)' name='txtConsultaPeriodo2' id='txtConsultaPeriodo2' class='form-control' style='font-size:12pt; padding:20px;'>
 ...
 ECHO   $param1 = $_GET['param1'];
    ...
    if ($param1 == 1) {
      echo "
                <div class='col-sm-12 col-xl-12'>
                        <label>Data</label>
                        <input onkeyup='validacaorelatorios(47, txtConsultaPeriodo2.value, txtTipopagamentoAvista2.value, txtTipopagamento2.value, this.value, txtTipoRelatorio222.value, 0, 47)' class='form-control' style='font-size:12pt; padding:20px;' type='date' id='txtDataParaPesquisa' name'txtDataParaPesquisa' value='" . date('Y-m-d') . "' />
                      </div>  
                       <div id='ResultadoValidacao47'> 
                       </div>
              ";
    ...
    if ($param1 == 2) {
      echo "
                 <div class='col-sm-12 col-xl-12'>
              <label>Mês/Ano</label>
              <input onkeyup='validacaorelatorios(47, txtConsultaPeriodo2.value, txtTipopagamentoAvista2.value, txtTipopagamento2.value, this.value, txtTipoRelatorio222.value, 0, 47)' class='form-control' style='font-size:12pt; padding:20px;' type='month' id='txtDataParaPesquisa' name'txtDataParaPesquisa' value='" . date('Y-m') . "' />
            </div> 
             <div id='ResultadoValidacao47'> 
              </div>
              ";
...
";
  ...
  case 51:
    ...
    $sqlPedidos22 = "SELECT * FROM pag_par_pro WHERE mes = :data ORDER BY cod ASC";
    $paramPedidos22 = array(
      ":data" => $mes_hoje
    );
    $parcela_areceber = 0;
    ...
    foreach ($dataTablePedidos22 as $resultadopedidos22) {
      ...
      if ($statusparcela == 1) {
        $parcela_areceber = $parcela_areceber + $resultadopedidos22['valor'];
      } else {

        $parcela_paga = $parcela_paga + $resultadopedidos22['valor'];
      }

      $parcela_total = $parcela_total + $resultadopedidos22['valor'];
    }
...
<table class='table table-bordered' style='width: 100%;'>  
  ...
  ";
    ...
    <table class='table table-bordered' style='width: 100%;'>
      <tr>
        <td><b>Parcelas Pagas</b></td>
        <td><b>Parcelas a Receber</b></td>
        <td><b>Total</b></td>
      </tr>
      <tr>
        <td>
             R$ " . number_format($parcela_paga, 2, ',', '.') . "
       </td>
        <td>
             R$ " . number_format($parcela_areceber, 2, ',', '.') . "
       </td>
        <td>
             R$ " . number_format($parcela_total, 2, ',', '.') . "
       </td>
      </tr>
    </table>
    ...
    while ($financeirocli = mysqli_fetch_object($sql)) {
      ...
      $sqlPar = mysqli_query($conn, "SELECT * FROM pag_par_pro WHERE esp_proc = $codfin AND status = 2 ORDER BY cod ASC");
      // Exibe todos os valores encontrados
      while ($finpar = mysqli_fetch_object($sqlPar)) {
        $contadorparcelas++;
      }
      ...
      if ($numparcelas != $contadorparcelas) {
        ...
        $sqlPar = mysqli_query($conn, "SELECT * FROM pag_par_pro WHERE esp_proc = $codfin ORDER BY cod ASC");
        // Exibe todos os valores encontrados
        while ($finpar = mysqli_fetch_object($sqlPar)) {
    ...
    while ($financeirocli = mysqli_fetch_object($sql)) {
      ...
      $sqlPar = mysqli_query($conn, "SELECT * FROM pag_par_pro WHERE esp_proc = $codfin AND status = 2 ORDER BY cod DESC");
      // Exibe todos os valores encontrados
      while ($finpar = mysqli_fetch_object($sqlPar)) {
        $valortotalparcelas = $valortotalparcelas + (float) $finpar->valor;
        $diapag = $finpar->dia;
  ...
  ";
    ...
    echo "<table class='table table-borbored'>
      <tr>
        <td><b>Categoria</b></td>
...
";
  ...
  case 66:
    ...
    if ($param1 == 1) {
      ...
      $mes_hoje = $partes['1'];
      $ano_hoje = $partes['0'];
      echo "<h3>Lista de Saídas Por Mês: " . mostraMes($mes_hoje) . "/$ano_hoje</h3>";
  ...
  case 68:
    ...
    if ($param5 == 0) {
      echo "<h1>INFORMAÇÕES RESUMIDAS</h1>";
      if ($param2 == 0) {
        $sqlCategorias = "SELECT * FROM lc_cat ORDER BY id ASC";
        $dataTableCategorias = $banco->ExecuteQuery($sqlCategorias);
     ...
     ";
      ...
      foreach ($dataTableCategorias as $resultadocategorias) {
        ...
        foreach ($dataTableFinEmpresa as $resultadofimempresa) {
          $status = $resultadofimempresa['status'];
          $cod = $resultadofimempresa['id'];
          $valor_total = $resultadofimempresa['valor'];
  ...
  case 75:
    ...
    $query2 = mysqli_query($conn, "UPDATE `pag_par_pro` SET `status` = '2' WHERE `pag_par_pro`.`cod` = $valor;");

    if ($query2) {
      //$sql = mysqli_query($conn, "SELECT * FROM pedidos WHERE usuario = " . $codnota . " ORDER BY cod ASC");
      $sqlPedidos22 = "SELECT * FROM pag_par_pro WHERE cod = :cod ORDER BY cod ASC";
      $paramPedidos22 = array(
        ":cod" => $valor
      );
...
<form onsubmit="return ConfirmarIsso();" method="post" name="frmCadastroAlterCli" id="frmCadastroAlterCli" class="needs-validation" novalidate enctype="multipart/form-data">
  <div class="row g-3">
    ...
    <div class="col-md-6">
      ...
      <input tabindex="12" type="text" class="form-control form-control-lg" id="txtBairro" name="txtBairro" value="' . $bairrocliente . '">
    </div>
...
';
  ...
  case 79:
    ...
    echo " 
       ...
       ";
    $sql = mysqli_query($conn, "SELECT * FROM financeiro_clientes WHERE categoria = $codcli ORDER BY cod ASC");
    // Exibe todos os valores encontrados
    $valortotalfinanceiro2 = 0;
    while ($financeirocli = mysqli_fetch_object($sql)) {
